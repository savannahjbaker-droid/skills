...
seamlessly with **ATS, EMR, and Salesforce** and connects directly to
**NPPES, CAQH, PECOS, and state medical boards** so provider data never has to
be re-keyed between systems.

This is an **MVP prototype**: a few core endpoints and a couple of real-shaped
connectors, structured so the remaining ~2000 PSV sources and outbound
integrations plug into the same interfaces without touching the core.
...
# 2. Run primary source verification across all sources
curl -s -X POST localhost:8000/providers/<PROVIDER_ID>/verify

# 3. Inspect the credentialing case
...
```
  ATS / EMR / Salesforce                 NPPES · CAQH · PECOS · State boards
          │  (native payloads)                     ▲  (PSV queries)
          ▼                                         │
  ┌───────────────┐   canonical    ┌────────────────────────────┐
...
- **One `PrimarySourceConnector` interface.** Every source — from NPPES to the
  2000th PSV database — implements the same `verify(provider) -> result`. The
